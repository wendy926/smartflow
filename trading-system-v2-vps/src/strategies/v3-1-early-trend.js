...
class EarlyTrendDetector {
  constructor(params = {}) {
    ...
    this.params = {
      ...
      weightBonus: params.weightBonus || 0.2, // 进一步优化信号质量
      delayBars: params.delayBars || 1, // 保持有效配置
      macdDivergence: params.macdDivergence || false // 保持有效配置
    };
  }
   ...
   * @private
   */
  _checkDelayConfirmation(klines1H, signalType) {
    if (!this.params.delayBars || this.params.delayBars <= 0) {
      return true; // 如果未设置延迟，直接返回true
    }

    const requiredBars = this.params.delayBars;
    if (klines1H.length < requiredBars + 1) {
      return false; // 数据不足
    }

    const closes = klines1H.map(k => parseFloat(k[4]));
    const recentBars = closes.slice(-requiredBars - 1);

    // 检查最近几根K线是否保持趋势方向
    let confirmationCount = 0;

    for (let i = 1; i < recentBars.length; i++) {
      const priceChange = recentBars[i] - recentBars[i - 1];

      if (signalType === 'EARLY_LONG' && priceChange > 0) {
        confirmationCount++;
      } else if (signalType === 'EARLY_SHORT' && priceChange < 0) {
        confirmationCount++;
      }
    }

    const confirmationRate = confirmationCount / requiredBars;
    const confirmed = confirmationRate >= 0.6; // 60%以上的K线保持趋势方向

    logger.debug(`延迟确认检查: 需要${requiredBars}根K线, 确认${confirmationCount}根, 确认率${(confirmationRate * 100).toFixed(1)}%, 结果=${confirmed}`);
