...
class DynamicStopLossManager {
  constructor(params = {}) {
    ...
    this.params = {
      // 初始止损ATR倍数（优化至3:1盈亏比）
      kEntryHigh: params.kEntryHigh || 0.2,
      kEntryMed: params.kEntryMed || 0.3,
      kEntryLow: params.kEntryLow || 0.4,

      // 趋势确认后扩大止损
      kHold: params.kHold || 0.5,

      // 时间止损
      timeStopMinutes: params.timeStopMinutes || 10,

      // 追踪止盈参数
      profitTrigger: params.profitTrigger || 0.2, // 盈利达到0.2x止损距离时启用
      trailStep: params.trailStep || 0.1,         // 追踪步长(ATR倍数)

      // 止盈因子（实现3:1盈亏比）
      tpFactor: params.tpFactor || 5.0
    };
  }
