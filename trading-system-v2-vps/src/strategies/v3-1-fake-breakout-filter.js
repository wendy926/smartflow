...
class FakeBreakoutFilter {
  constructor(params = {}) {
    // 默认参数（优化至3:1盈亏比配置）
    this.params = {
      volFactor: params.volFactor || 0.2,        // 成交量因子（大幅放宽）
      deltaThreshold: params.deltaThreshold || 0.0005, // Delta阈值（大幅放宽）
      confirmBars: params.confirmBars || 1,      // 确认K线数
      reclaimPct: params.reclaimPct || 0.001,   // 回撤百分比(0.1%，大幅放宽)
      rangeLookback4H: params.rangeLookback4H || 2, // 4H区间回溯
      trendScoreThreshold: params.trendScoreThreshold || 0.5 // 趋势评分阈值（大幅放宽）
    };
  }
   ...
   * @param {number} trendScore - 趋势评分（可选）
   * @returns {Object} 过滤结果
   */
  filterTrend(breakoutPrice, klines15m, klines1H, klines4H, trendScore = 0) {
    try {
      ...
      // 综合判断：根据趋势评分决定过滤严格程度
      let passed;
      if (trendScore >= this.params.trendScoreThreshold) {
        // 强趋势时弱化过滤：只检查关键指标
        passed = volumeCheck.passed && deltaCheck.passed;
        logger.info(`强趋势弱化过滤: 趋势评分=${trendScore}, 只检查成交量和Delta`);
      } else {
        // 普通情况：放宽过滤条件，只检查最关键的指标
        passed = volumeCheck.passed && deltaCheck.passed;
        logger.info(`普通情况放宽过滤: 只检查成交量和Delta`);
      }
  ...
  _checkDelta(klines15m, klines1H) {
    ...
    // 大幅放宽Delta要求：只检查同向，不检查强度
    const sameDirection = Math.sign(delta15m) === Math.sign(delta1H);
    const strongEnough = Math.abs(delta15m) >= 0.001; // 降低到0.001
