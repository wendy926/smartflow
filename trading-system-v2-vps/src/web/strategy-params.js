...
class StrategyParamsManager {
  ...
  async runBacktest(strategy) {
    ...
    try {
      ...
      const backtestPromises = modes.map(mode =>
        this.startSingleModeBacktest(strategy, mode)
      );
      ...
      if (successCount > 0) {
        alert(`回测任务已启动: ${successCount}个模式成功, ${failedCount}个模式失败`);
   ...
   */
  async startSingleModeBacktest(strategy, mode) {
    try {
      ...
      if (result.success) {
        console.log(`[策略参数] ${strategy}-${mode}回测任务已启动`);
        return result;
      } else {
