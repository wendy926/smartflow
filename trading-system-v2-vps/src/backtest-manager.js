...
class BacktestManager {
  ...
  async fetchMarketData(symbols, timeframe = '1h') {
    ...
    try {
      // 并行获取所有交易对的数据
      const dataPromises = symbols.map(symbol => this.fetchSymbolData(symbol, timeframe));
      ...
      logger.info(`[回测管理器] 获取到${Object.keys(marketData).length}个交易对的${timeframe}数据`);
      return marketData;
  ...
  async fetchSymbolData(symbol, timeframe = '1h') {
    try {
      logger.info(`[回测管理器] 开始获取${symbol}的${timeframe}数据`);
  ...
  async getCachedMarketData(symbol, timeframe = '1h') {
    try {
      logger.info(`[回测管理器] 从数据库获取${symbol}的${timeframe}缓存数据`);
      ...
      logger.info(`[回测管理器] 查询到${rows.length}条${symbol}-${timeframe}数据`);
