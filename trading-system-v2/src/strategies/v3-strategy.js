...
class V3Strategy {
  ...
  updateDrawdownStatus(pnl) {
    ...
    logger.info(`V3策略回撤更新: 当前权益=${this.currentEquity.toFixed(2)}, 峰值权益=${this.peakEquity.toFixed(2)}, 当前回撤=${(currentDrawdown * 100).toFixed(2)}%, 最大回撤=${(this.maxDrawdown * 100).toFixed(2)}%`);
  }
  ...
  async calculateTradeParameters(symbol, signal, currentPrice, atr, marketType = 'RANGE', confidence = 'med') {
    try {
      ...
      if (currentDrawdown > maxDrawdownLimit) {
        logger.warn(`${symbol} V3策略: 当前回撤${(currentDrawdown * 100).toFixed(2)}%超过限制${(maxDrawdownLimit * 100).toFixed(1)}%，暂停交易`);
  ...
  combineSignals(trend4H, factors1H, execution15M) {
    ...
    if (normalizedScore >= 30 && trendDirection !== 'RANGE') {
      ...
      if (satisfiedCount >= 2) {  // 至少满足2个条件
        logger.info(`✅ 强信号触发(新逻辑): 总分=${normalizedScore}%, 趋势=${trendScore}>=${trend4HModerateThreshold}, 因子=${factorScore}>=${factorModerateThreshold}, 15M=${entryScore}>=${entry15MModerateThreshold}, 满足${satisfiedCount}个条件`);
    ...
    if (normalizedScore >= 20 && normalizedScore < 30 && trendDirection !== 'RANGE') {
      if (trendScore >= trend4HModerateThreshold || factorScore >= factorModerateThreshold || entryScore >= entry15MModerateThreshold) {
        logger.info(`⚠️ 中等信号触发(新逻辑): 总分=${normalizedScore}%, 趋势=${trendScore}>=${trend4HModerateThreshold}, 因子=${factorScore}>=${factorModerateThreshold}, 15M=${entryScore}>=${entry15MModerateThreshold}`);
        return trendDirection === 'UP' ? 'BUY' : 'SELL';
      }
    }
