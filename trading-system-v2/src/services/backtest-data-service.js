...
class BacktestDataService {
  ...
  async getHistoricalData(symbol, timeframe = '1h', forceRefresh = false) {
    ...
    try {
      ...
      // 直接从数据库获取预存储的数据
      logger.info(`[回测数据服务] 从数据库获取${symbol}-${timeframe}的预存储数据`);
      ...
      if (dbData && dbData.length > 0) {
        ...
        logger.info(`[回测数据服务] 成功从数据库获取${symbol}-${timeframe}数据: ${dbData.length}条`);
        return dbData;
      }

      // 如果数据库没有数据，抛出错误提示需要预加载
      throw new Error(`数据库中没有${symbol}-${timeframe}的预存储数据，请先运行数据预加载`);
