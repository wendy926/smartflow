...
class MarketDataPreloader {
   ...
   * @returns {Promise<Object>} 预加载结果
   */
  async preload2024Data(symbols = ['BTCUSDT', 'ETHUSDT'], timeframes = ['1h', '5m']) {
    const startTime = new Date('2024-01-01').getTime();
    const endTime = new Date('2024-12-31').getTime();

    logger.info(`[数据预加载] 开始预加载2024年数据: ${symbols.join(', ')} - ${timeframes.join(', ')}`);
    ...
    try {
      ...
      for (const symbol of symbols) {
        for (const timeframe of timeframes) {
          allTasks.push(this.preloadDataByRange(symbol, timeframe, startTime, endTime));
        }
      }
      ...
      taskResults.forEach((result, index) => {
        const symbolIndex = Math.floor(index / timeframes.length);
        ...
        if (result.status === 'fulfilled') {
          ...
          results.failedTasks++;
          results.details[`${symbol}-${timeframe}`] = { success: false, error: result.reason.message };
        }
      });

      const duration = Date.now() - Date.now();
      logger.info(`[数据预加载] 2024年数据预加载完成: ${results.completedTasks}/${results.totalTasks} 成功, 耗时: ${duration}ms`);

      return results;
    } catch (error) {
      logger.error('[数据预加载] 2024年数据预加载失败:', error);
