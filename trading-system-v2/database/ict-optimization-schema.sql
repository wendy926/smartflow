...
-- 7. 创建视图用于分析ICT策略性能
CREATE OR REPLACE VIEW ict_strategy_performance AS
SELECT 
    s.symbol,
    sj.strategy_name,
    sj.timeframe,
    COUNT(*) as total_signals,
    SUM(CASE WHEN sj.entry_signal != 'HOLD' THEN 1 ELSE 0 END) as active_signals,
    AVG(sj.confidence_score) as avg_confidence,
    AVG(sj.harmonic_score) as avg_harmonic_score,
    AVG(sj.engulfing_strength) as avg_engulfing_strength,
    SUM(CASE WHEN sj.gate_passed = TRUE THEN 1 ELSE 0 END) as gate_passed_count,
    SUM(CASE WHEN sj.secondary_passed = TRUE THEN 1 ELSE 0 END) as secondary_passed_count,
    SUM(CASE WHEN sj.confirmation_status = 'CONFIRMED' THEN 1 ELSE 0 END) as confirmed_count,
    AVG(sj.adaptive_stop_multiplier) as avg_stop_multiplier,
    AVG(sj.position_size_usd) as avg_position_size
FROM strategy_judgments sj
JOIN symbols s ON sj.symbol_id = s.id
WHERE sj.strategy_name = 'ICT'
GROUP BY s.symbol, sj.strategy_name, sj.timeframe
ORDER BY total_signals DESC;

-- 8. 创建存储过程用于计算历史胜率
DELIMITER //
CREATE PROCEDURE CalculateICTWinRate(
    IN p_symbol_id INT,
    IN p_days_back INT
)
BEGIN
    ...
    SET v_period_start = DATE_SUB(v_period_end, INTERVAL p_days_back DAY);
    
    INSERT INTO ict_win_rate_history (
        symbol_id, timeframe, period_start, period_end,
        total_trades, winning_trades, losing_trades, win_rate,
        avg_pnl, total_pnl, max_drawdown, sharpe_ratio
    )
    SELECT 
        p_symbol_id,
        '15M',
        ...
        v_period_end,
        COUNT(*),
        SUM(CASE WHEN st.pnl > 0 THEN 1 ELSE 0 END),
        SUM(CASE WHEN st.pnl < 0 THEN 1 ELSE 0 END),
        CASE WHEN COUNT(*) > 0 THEN SUM(CASE WHEN st.pnl > 0 THEN 1 ELSE 0 END) / COUNT(*) ELSE 0 END,
        AVG(st.pnl),
        SUM(st.pnl),
        0, -- 需要单独计算最大回撤
        0  -- 需要单独计算夏普比率
    FROM simulation_trades st
    WHERE st.symbol_id = p_symbol_id 
    AND st.strategy_name = 'ICT'
    AND st.entry_time >= v_period_start
    AND st.entry_time <= v_period_end;
END //
DELIMITER ;
