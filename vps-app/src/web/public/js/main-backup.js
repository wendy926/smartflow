...
class SmartFlowApp {
  ...
  setupEventListeners() {
    ...
    if (ictMaxLossAmountElement) {
      ictMaxLossAmountElement.addEventListener('change', async (e) => {
        const value = e.target.value;
        await this.saveUserSetting('maxLossAmount', value);
        console.log('💰 ICT最大损失金额已更新为:', value, 'USDT');
    ...
    // ICT策略刷新按钮
    const ictRefreshBtn = document.getElementById('ictRefreshBtn');
    if (ictRefreshBtn) {
      ictRefreshBtn.addEventListener('click', () => {
        this.refreshICTSignals();
      });
    }

    // ICT策略清除缓存按钮
    const ictClearCacheBtn = document.getElementById('ictClearCacheBtn');
    if (ictClearCacheBtn) {
      ictClearCacheBtn.addEventListener('click', () => {
        this.clearICTCache();
      });
    }
