...
#### GET /api/rollup-calculator
- **功能**: 获取动态杠杆滚仓计算器页面
- **实现文件**: `public/rollup-calculator.html`
- **入参**: 无
- **出参**: HTML页面

**页面功能**:
- 初单计算：用户输入最大损失金额、当前价格、止损价格、目标价格
- 动态杠杆滚仓：基于rolling-v1.md文档的完整算法实现
- 参数配置：滚仓触发比例、杠杆递减系数、落袋比例、最低杠杆
- 实时计算：100步价格模拟，精确计算每个阶段
- 结果展示：动态杠杆滚仓路径表和滚仓详情表
...
**策略步骤**:
1. **Step 1：初始开仓** - 使用初单计算的本金和杠杆
2. **Step 2：第一阶段盈利触发** - 浮盈≥本金×触发比例时抽回本金
3. **Step 3：后续滚仓阶段** - 杠杆递减，落袋部分利润，剩余浮盈滚仓
4. **Step 4：趋势结束** - 全部盈利落袋，确保本金安全

**参数说明**:
- `triggerRatio`: 浮盈达到本金的多少比例时触发滚仓
- `leverageDecay`: 每次滚仓后杠杆的递减比例
- `profitLockRatio`: 每次滚仓时锁定利润的比例
- `minLeverage`: 滚仓过程中杠杆的最低限制
...
### V3.17 (2025-01-07)
- **动态杠杆滚仓计算器优化** - 完全重构滚仓计算器，实现基于rolling-v1.md文档的动态杠杆滚仓方案
- **保留初单计算模块** - 用户输入最大损失金额、当前价格、止损价格、目标价格
- **移除对比杠杆策略** - 专注于动态杠杆滚仓方案，提升用户体验
- **实现动态杠杆滚仓算法** - 基于文档中的Step 1-4完整实现simulateDynamicPyramid函数
- **参数化配置** - 添加滚仓触发比例(0.5-2.0)、杠杆递减系数(0.3-0.7)、落袋比例(0.3-0.7)、最低杠杆(3-15)选项
- **智能滚仓机制** - 自动判断触发条件，执行本金抽回、利润落袋、杠杆递减、剩余浮盈滚仓
- **风险控制优化** - 本金安全、利润锁定、杠杆递减、止损保护多重保障
- **实时计算展示** - 100步价格模拟，动态杠杆滚仓路径表和滚仓详情表
- **单元测试完善** - 新增20个测试用例，覆盖基础功能、算法逻辑、错误处理、性能测试
- **API文档更新** - 新增动态杠杆滚仓计算器API说明和核心算法文档
- **前端界面优化** - 更新HTML结构和JavaScript逻辑，提升用户交互体验

### V3.18 (2025-01-14)
- **MA数据新鲜度修复** - 解决MA20、MA50、MA200数据严重过期问题，确保策略基于最新数据计算
- **智能数据获取机制** - StrategyV3Core新增getKlineData方法，优先使用新鲜数据库数据，过期时自动获取实时数据
- **数据新鲜度检查** - 新增checkDataFreshness方法，支持不同时间间隔的阈值检查（4H:8小时，1H:2小时，15m:30分钟）
- **异步数据库更新** - 新增updateDatabaseAsync方法，异步更新K线数据不阻塞策略分析
- **服务器端自动更新** - server.js新增定期K线数据更新任务，每30分钟自动更新所有交易对的K线数据
- **数据更新API** - 新增updateKlineData方法，支持4h、1h、15m三个时间间隔的数据更新
- **单测覆盖完善** - 新增ma-data-freshness-fix.test.js（27个测试用例）和server-kline-update.test.js（19个测试用例）
- **内存泄漏修复** - 修复单测中的定时器和数据库连接内存泄漏问题，确保测试进程正常退出
- **错误处理增强** - 完善的错误处理机制，API失败时回退到数据库数据，数据库更新失败不影响策略分析
- **性能优化** - 避免不必要的API调用和数据库写入，提升系统整体性能
- **数据一致性保障** - 确保所有交易对使用最新的K线数据，提高趋势判断和MA计算的准确性
